...
function NovaAvaliacao() {
  ...
  const carregarDados = async () => {
    try {
      ...
      // Carregar funcionários, avaliadores e questionários em paralelo
      const [funcionariosData, avaliadoresData, questionariosData] = await Promise.all([
        api.getFuncionarios(null, null, 1, 1000), // Buscar todos os funcionários (até 1000)
        api.getAvaliadores(),
        api.getQuestionarios()
      ])

      // Extrair funcionários da resposta (pode vir paginada ou como array)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Resultado da criação da avaliação:', resultado)
      
      // Obter o ID da avaliação criada
      // O backend retorna um objeto com cod_avaliacao
      const avaliacaoId = resultado?.cod_avaliacao || resultado?.id
      
      console.log('ID da avaliação obtido:', avaliacaoId)
      
      if (avaliacaoId) {
        // Redirecionar para a tela de preenchimento do questionário
        console.log('Redirecionando para:', `/avaliacoes/${avaliacaoId}/preencher`)
        navigate(`/avaliacoes/${avaliacaoId}/preencher`)
      } else {
        // Fallback: mostrar toast e redirecionar para listagem
        console.error('ID da avaliação não encontrado no resultado:', resultado)
        showToast('error', 'Erro ao obter ID', 'A avaliação foi criada, mas não foi possível obter o ID. Verifique na listagem.')
        setTimeout(() => {
          navigate('/avaliacoes')
        }, 2000)
      }
      ...
      const errorMessage = error.response?.data?.error || error.message || 'Não foi possível criar a avaliação. Tente novamente.'
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p>Preencha os dados para iniciar uma nova avaliação</p>
        </div>
      </div>
