...
function QuestionariosBarChart({ data }) {
  const svgRef = useRef()
  ...
  useEffect(() => {
    ...
    const yScale = d3.scaleLinear()
      .domain([0, d3.max(data, d => d.total_usos || 0)])
      .nice()
      .range([innerHeight, 0])
    ...
    g.selectAll(".bar")
      ...
      .attr("y", d => yScale(d.total_usos || 0))
      .attr("width", xScale.bandwidth())
      .attr("height", d => innerHeight - yScale(d.total_usos || 0))
      .attr("fill", d => gradient(d.total_usos || 0))
      .attr("rx", 4)
      .on("mouseover", function(event, d) {
        ...
        tooltip.html(`
          ...
          Usos: ${d.total_usos || 0}<br/>
          ${d.media_rating ? `Média Rating: ${d.media_rating.toFixed(1)}` : ''}
        `)
    ...
    // Adicionar valores nas barras (brancos quando sobrepõem a barra)
    g.selectAll(".bar-label")
      ...
      .attr("y", d => {
        const barHeight = innerHeight - yScale(d.total_usos || 0)
        // Se a barra for alta o suficiente, colocar o rótulo dentro (branco)
        // Caso contrário, colocar acima (preto)
        return barHeight > 25 
          ? yScale(d.total_usos || 0) + 15  // Dentro da barra
          : yScale(d.total_usos || 0) - 5   // Acima da barra
      })
  ...
  return (
    <div className="chart-container" ref={containerRef} style={{ width: '100%', height: '100%', minHeight: '400px' }}>
      <h3 style={{ marginBottom: '20px', fontSize: '18px', fontWeight: 600 }}>
        Frequência de Uso dos Questionários
      </h3>
...
export default QuestionariosBarChart
